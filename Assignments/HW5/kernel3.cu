    ...
    int max_iterations, int *output, size_t pitch_in_ints)
{
    ...
    int base_x = (blockIdx.x * blockDim.x + threadIdx.x) * GROUP_SIZE;
    int base_y = (blockIdx.y * blockDim.y + threadIdx.y) * GROUP_SIZE;
    

    for (int dy_offset = 0; dy_offset < GROUP_SIZE; ++dy_offset) {
        int y = base_y + dy_offset;
        if (y >= height) continue;

        for (int dx_offset = 0; dx_offset < GROUP_SIZE; ++dx_offset) {
            int x = base_x + dx_offset;
            if (x >= width) continue;

            float cx = x0 + x * dx;
            float cy = y0 + y * dy;

            int index = y * pitch_in_ints + x;
            output[index] = mandel(cx, cy, max_iterations);
        }
    }
}
