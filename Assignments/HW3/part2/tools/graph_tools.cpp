...
const std::string CMD_TEXT2BIN = "text2bin";
const std::string CMD_INFO = "info";
const std::string CMD_PRINT = "print";
const std::string CMD_NOOUTEDGES = "noout";
const std::string CMD_NOINEDGES = "noin";
const std::string CMD_EDGESTATS = "edgestats";
...
int main(int argc, char **argv)
{
    ...
    if (cmd == CMD_TEXT2BIN)
    {
        ...
        Graph g;
        std::cout << "Loading graph: " << input_filename << "\n";
        g = load_graph(input_filename.c_str());
        std::cout << "Done loading.\n";
        store_graph_binary(output_filename.c_str(), g);
        free_graph(g);
    }
    else if (cmd == CMD_INFO)
    {
        ...
        Graph g;
        std::cout << "Loading graph: " << input_filename << "\n";
        g = load_graph_binary(input_filename.c_str());
        std::cout << "Done loading.\n";
    ...
    else if (cmd == CMD_PRINT)
    {
        ...
        {
            std::cerr << "Usage: " << argv[0] << " " << cmd << " filename\n";
            std::cerr << "Pretty-prints graph, including edge information (be careful with large "
                         "graphs)\n";
            exit(1);
        }
    ...
    else if (cmd == CMD_EDGESTATS)
    {
        ...
        double avg_outgoing = (double)total_outgoing / num_nodes(g);
        double avg_incoming = (double)total_incoming / num_nodes(g);

        std::cout << "=========================================================\n";
        std::cout << "Edge statistics for this graph:\n";
        std::cout << "=========================================================\n";
        std::cout << "The graph " << ((is_symmetric) ? "IS " : "IS NOT ") << "symmetric.\n";
        std::cout << "Outgoing edges: total=" << total_outgoing << " avg=" << avg_outgoing
                  << " min=" << min_outgoing << " max=" << max_outgoing << "\n";

        std::cout << "Incoming edges: total=" << total_incoming << " avg=" << avg_incoming
                  << " min=" << min_incoming << " max=" << max_incoming << "\n";
    }
